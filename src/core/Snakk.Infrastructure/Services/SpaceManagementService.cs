    ...
    ILogger<SpaceManagementService> _logger) : ISpaceManagementService
{
    ...
    public async Task<SpaceRulesDto> GetRulesAsync(
        string spaceId,
        CancellationToken cancellationToken = default)
    {
        var rules = await context.SpaceRules
    ...
    public async Task<SpaceRulesDto> UpdateRulesAsync(
        string spaceId,
        UpdateSpaceRulesRequest request,
        CancellationToken cancellationToken = default)
    {
        var space = await context.Spaces
            .AsTracking()
        ...
        if (space is null)
            return new SpaceRulesDto { Rules = [] };

        var existingRules = await context.SpaceRules
            .Where(r => r.SpaceId == space.Id)
            .ToListAsync(cancellationToken);

        context.SpaceRules.RemoveRange(existingRules);

        var now = DateTime.UtcNow;

        var newRules = request.Rules
            .Select((r, index) => new SpaceRuleDatabaseEntity
            {
                SpaceId = space.Id,
                Title = r.Title,
                Description = r.Description,
        ...
        context.SpaceRules.AddRange(newRules);
