    ...
    ILogger<MediaService> logger) : IMediaService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Medium thumbnail — bounded-box resize: cap the longest side at 900px
            // so both very wide (landscape) and very tall (portrait) images are
            // constrained. ResizeMode.Max preserves aspect ratio and fits within
            // the 900×900 box without cropping. The width-or-height guard mirrors
            // the small-thumb tier and prevents upscaling small source images.
