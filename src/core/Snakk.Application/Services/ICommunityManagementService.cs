...
public interface ICommunityManagementService
{
    ...
    Task<CommunitySettingsDto?> UpdateSettingsAsync(string communityId, UpdateCommunitySettingsRequest request, CancellationToken cancellationToken = default);

    Task<CommunityModerationDto> GetModerationDataAsync(string communityId, CancellationToken cancellationToken = default);

    Task<CommunityMembersListDto> GetMembersAsync(string communityId, int page = 1, int pageSize = 50, CancellationToken cancellationToken = default);

    Task<bool> UpdateMemberRoleAsync(string communityId, string userId, UpdateMemberRoleRequest request, CancellationToken cancellationToken = default);
    ...
    Task<CommunityRulesDto> UpdateRulesAsync(string communityId, UpdateCommunityRulesRequest request, CancellationToken cancellationToken = default);
}
