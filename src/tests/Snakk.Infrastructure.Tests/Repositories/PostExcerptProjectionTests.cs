...
/// fix/post-card-stored-xss (originally fix/post-topic-item-stored-xss against the pre-v0.7.002 _PostTopicItem partial). The reactions / saved-posts projections used to
/// emit the raw markdown source (`Post.Content.Substring(0, 300)`) as the excerpt,
/// which `_PostCard.cshtml` then rendered with `@Html.Raw` — so a post body
...
public class PostExcerptProjectionTests : IDisposable
{
    ...
    public PostExcerptProjectionTests()
    {
        ...
        _saves = new SaveRepository(_db.Context, cache);
    }
