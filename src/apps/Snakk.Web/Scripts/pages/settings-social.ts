(function () {
    ...
    function buildLinkRow(link: SocialLink): HTMLElement {
        ...
        row.className = 'social-link-row flex items-center gap-3 p-3 rounded-lg';
        ...
        nameSpan.className = 'text-sm font-medium w-28 shrink-0';
        nameSpan.textContent = link.displayName;

        const valueEl = document.createElement('span');
        valueEl.className = 'text-sm flex-1 truncate';
        if (link.url) {
        ...
        removeBtn.className = 'btn btn-ghost btn-xs shrink-0';
    ...
    async function saveLinks(): Promise<void> {
        ...
        try {
            ...
            setTimeout(() => saveSuccessEl.classList.add('hidden'), 3000);
        } catch {
