...
interface FrontpageDiscussionsOptions {
    pageSize?: number;
    scrollSentinelMargin?: string;
...
interface Discussion {
    ...
    postCount: number;
    hub: { slug: string; publicId: string; name: string; avatarUrl?: string; avatarFileName?: string };
    space: { slug: string; publicId: string; name: string; avatarUrl?: string; avatarFileName?: string };
    community?: { slug: string; publicId: string; name: string; avatarUrl?: string; avatarFileName?: string };
}

class FrontpageDiscussions {
    private pageSize: number;
    private scrollSentinelMargin: string;
    private previewMaxLength: number;
    private nextCursor: string;
    private hasMore: boolean;
    private scrollObserver: IntersectionObserver | null;
    private loadMoreRequest: Promise<Response> | null;
    private previewHandlersAttached: boolean;
    ...
    constructor(options: FrontpageDiscussionsOptions = {}) {
        ...
        this.previewCache = new Map<string, string>();
     ...
     */
    async fetchPreview(discussionId: string): Promise<string | null> {
     ...
     * @param {string} discussionId
     */
    async togglePreview(button: HTMLElement, previewDiv: HTMLElement, discussionId: string): Promise<void> {
     ...
     */
    attachPreviewHandlers(container: HTMLElement): void {
     ...
     */
    createDiscussionElement(discussion: Discussion): DocumentFragment {
    ...
    async loadMore(): Promise<void> {
        ...
        if (this.loadMoreRequest || !this.hasMore) return;
        ...
        try {
            ...
            const data = await response.json() as { items: Discussion[]; nextCursor?: string; hasMoreItems: boolean };

            if (data.items && data.items.length > 0) {
                data.items.forEach((discussion: Discussion) => {
