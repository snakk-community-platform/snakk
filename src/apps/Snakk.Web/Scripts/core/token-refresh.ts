...
(function() {
    ...
    async function checkAndRefreshToken(): Promise<void> {
        try {
            ...
            const token = (window as any).SnakkAuth?.getToken();
            if (!token) {
            ...
            if (!shouldRefresh) {
                const payload = (window as any).SnakkAuth?.parseJwt(token);
                if (payload?.exp) {
            ...
            const success = await (window as any).SnakkAuth?.refreshTokens();
            ...
            dispatchTokenEvent('error', { error: (error as Error).message });
        } finally {
            isRefreshing = false;
        }
    }
     ...
     */
    function dispatchTokenEvent(type: string, detail: any = {}): void {
    ...
    (window as any).SnakkTokenRefresh = {
        checkAndRefreshToken,
        startTokenRefresh,
        stopTokenRefresh,
