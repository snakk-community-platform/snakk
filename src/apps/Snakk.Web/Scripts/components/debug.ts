...
(function (): void {
    ...
    type Source = 'Snakk.Web' | 'Browser' | 'HTMX' | 'SignalR' | 'Error' | 'Context';

    // ========================================================================
    // State
    // ========================================================================

    const htmxStartTimes: Map<string, number> = new Map();
    let timeOrigin = 0;

    // Buffered entries for deferred output (before + during page load)
    const buffer: Array<{ source: Source; message: string; timestamp: number }> = [];
    let flushed = false;
    ...
    function emit(source: Source, message: string, timestamp: number): void {
        ...
        if (source === 'Error') {
            console.error('%cSNAKK%c %c%s%c %s %c%s',
                snakkBadge, resetStyle, badge, source, resetStyle, timeStr, msgStyle, message);
        } else {
            console.debug('%cSNAKK%c %c%s%c %s %c%s',
                snakkBadge, resetStyle, badge, source, resetStyle, timeStr, msgStyle, message);
        }
    }

    function add(source: Source, message: string, atTimestamp?: number): void {
    ...
    function log(source: Source, message: string, atTimestamp?: number): void {
    ...
    function logServerTimingEntry(t: ServerTimingEntry, requestBaseMs: number): void {
    ...
    function readContextEntries(): void {
    ...
    window.addEventListener('load', () => {
        ...
        if (navEntries.length > 0) {
            ...
            if (nav && 'serverTiming' in nav) {
                const timings = (nav as any).serverTiming as PerformanceServerTiming[];
                for (const t of timings) {
                    if (t.name === 'total' || t.name === 'cache') continue;
    ...
    // HTMX events
    let htmxPagePath: string | null = null;

    document.addEventListener('htmx:beforeRequest', ((e: CustomEvent) => {
    ...
    document.addEventListener('htmx:afterRequest', ((e: CustomEvent) => {
        const detail = e.detail;
    ...
    document.addEventListener('htmx:afterSettle', ((e: CustomEvent) => {
        ...
        const target = detail?.target as HTMLElement | undefined;
        const cacheDiv = target?.querySelector('[data-cache-source]') as HTMLElement | null;
