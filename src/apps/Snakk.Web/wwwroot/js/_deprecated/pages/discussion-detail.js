...
function createPostElement(post, isSameAuthorAsPrevious, currentUserId, isAuthenticated, isLocked) {
    ...
    if (!isSameAuthorAsPrevious) {
        ...
        if (post.author.role === 'admin') {
            rolesBadge = '<span class="badge badge badge-xs ml-1">Admin</span>';
        } else if (post.author.role === 'mod') {
            rolesBadge = '<span class="badge badge badge-xs ml-1">Mod</span>';
        }
        const opBadge = isOP ? '<span class="badge badge-primary badge-xs ml-1">OP</span>' : '';
        ...
        if (post.author.isDeleted) {
            ...
            authorBlockHtml = `
                <div class="author-block">
                    <div class="author-avatar-simple">
                        <img src="/storage/avatars/generated/users/${post.author.publicId}.svg" alt="${post.author.displayName}" loading="lazy" />
                    </div>
                    <div class="author-info">
...
document.addEventListener('input', (e) => {
    if (e.target.matches('textarea[data-auto-grow]')) {
        autoGrow(e.target);
    }
});
