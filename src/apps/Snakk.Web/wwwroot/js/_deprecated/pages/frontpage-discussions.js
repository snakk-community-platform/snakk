...
class FrontpageDiscussions {
    ...
    init() {
        ...
        this.scrollObserver = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting && !this.loadMoreRequest && this.hasMore) {
                this.loadMore();
            }
        }, {
    ...
    async fetchPreview(discussionId) {
        if (this.previewCache.has(discussionId)) {
            return this.previewCache.get(discussionId);
        }
    ...
    createDiscussionElement(discussion) {
        ...
        const spaceAvatarUrl = `/storage/avatars/generated/spaces/${discussion.space.publicId}.svg`;
    ...
    updateScrollCounter() {
        ...
        const totalDiscussions = container.querySelectorAll('.topic-item-wrapper').length;
        this.scrollCounter.textContent = totalDiscussions;
    }
