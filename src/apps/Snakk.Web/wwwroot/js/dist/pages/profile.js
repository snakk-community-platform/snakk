"use strict";(function(){"use strict";const f=window.SnakkUtils?.escapeHtml||function(a){if(!a)return"";const c=document.createElement("div");return c.textContent=a,c.innerHTML},E=window.DOMPurify,I=a=>a&&E?E.sanitize(a,{USE_PROFILES:{html:!0}}):"",C=window.SnakkUtils?.sanitizeUrl||function(a){if(!a)return"#";const c=a.trim().toLowerCase();return c.startsWith("javascript:")||c.startsWith("data:")?"#":a},L=window.SnakkUtils?.formatRelativeTime||function(a){const c=new Date(a),r=Math.floor((new Date().getTime()-c.getTime())/1e3);if(r<60)return"just now";if(r<3600)return`${Math.floor(r/60)}m ago`;if(r<86400)return`${Math.floor(r/3600)}h ago`;if(r<604800)return`${Math.floor(r/86400)}d ago`;const b=window.snakkTimezone||"UTC";try{return c.toLocaleDateString("en-US",{timeZone:b,month:"short",day:"numeric",year:"numeric"})}catch{return c.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}},M=document.getElementById("profile-page-config");if(!M)return;const p=JSON.parse(M.textContent||"{}").userId;function S(){async function a(){try{const t=await(await fetch(`/bff/users/${p}/stats`)).json(),s=document.getElementById("stat-followers");s&&(s.textContent=t.followerCount||0);const i=document.getElementById("stat-replies");i&&t.replyCount!==void 0&&(i.textContent=t.replyCount)}catch(n){console.error("Error loading user stats:",n);const t=document.getElementById("stat-followers");t&&(t.textContent="0")}}async function c(n){const t=document.getElementById("recent-posts");if(t)try{const i=await(await fetch(`/bff/search/posts?authorPublicId=${p}&pageSize=${n}`)).json();if(!i.items||i.items.length===0){t.innerHTML=`
                ...
                `).join("")}</div>`}catch(s){console.error("Error loading posts:",s),t.innerHTML='<div class="text-center py-8 text-error">Failed to load posts</div>'}}async function v(n){const t=document.getElementById("activity-chart-sidebar"),s=document.getElementById("activity-chart-main"),o=window.matchMedia("(min-width: 1024px)").matches?t:s;if(o)try{const $=((await(await fetch(`/bff/users/${p}/activity-history?days=${n}`)).json()).activities||[]).map(u=>({date:u.date,discussions:u.discussionCount??0,posts:u.postCount??0,total:(u.discussionCount??0)+(u.postCount??0)})),g=o.clientHeight,y=g>0?Math.max(g-40,60):150;r(o,$,n,y)}catch(w){console.error("Error loading activity chart:",w),o.innerHTML='<div class="text-center py-8 text-error">Failed to load activity chart</div>'}}function r(n,t,s,i=150){if(!t||t.length===0){n.innerHTML=`
                ...
                `;return}const o=Math.max(...t.map(e=>e.total),1),w=s>30;let x=t;if(w){const e=[];for(let l=0;l<t.length;l+=7){const d=t.slice(l,l+7);if(d.length===0||!d[0])continue;const k={date:d[0].date,discussions:d.reduce((m,h)=>m+h.discussions,0),posts:d.reduce((m,h)=>m+h.posts,0),total:d.reduce((m,h)=>m+h.total,0),isWeek:!0};e.push(k)}x=e}const $=x.map(e=>{const l=o>0?e.total/o*100:0,d=e.total>0?e.discussions/e.total*100:0,k=e.total>0?e.posts/e.total*100:0,h={timeZone:window.snakkTimezone||"UTC",month:"short",day:"numeric"},T=H=>{try{return new Date(H).toLocaleDateString("en-US",h)}catch{return new Date(H).toLocaleDateString("en-US",{month:"short",day:"numeric"})}},D=w?`Week of ${T(e.date)}`:T(e.date);return`
            ...
            `}async function b(){const n=document.getElementById("name-history-section"),t=document.getElementById("name-history-list");if(!(!n||!t))try{const i=await(await fetch("/bff/me/display-name-history",{credentials:"include"})).json();if(!i.entries||i.entries.length===0)return;t.innerHTML=i.entries.map(o=>`
                ...
                `).join(""),n.classList.remove("hidden")}catch(s){console.error("Error loading name history:",s)}}async function B(){const n=document.getElementById("profile-actions");if(n)try{const s=await(await fetch("/bff/auth/status",{credentials:"include"})).json();if(!s.isAuthenticated){n.innerHTML="";return}if(s.publicId===p){n.innerHTML=`
                    ...
                    `,b();return}const o=await(await fetch(`/bff/users/${p}/follow-status?currentUserId=${s.publicId}`,{credentials:"include"})).json();n.innerHTML=`
