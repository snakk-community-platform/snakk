...
(function () {
    ...
    function replyToPost(postId, authorName) {
        const replyToInput = document.getElementById('reply-to-post-id');
        ...
        if (replyContextAuthor)
            replyContextAuthor.textContent = authorName;
        if (textarea) {
            textarea.focus();
            autoGrow(textarea);
        }
    ...
    function showSelectionQuoteButton() {
        ...
        if (!selection || !selection.rangeCount || !currentSelection.text)
            return;
    ...
    function initDraftAutoSave(discussionId) {
        const textarea = document.getElementById('post-content-input');
        if (!textarea || !window.SnakkDraftManager)
            return;
        // Restore draft if exists
        const getReplyToPostId = () => {
            const input = document.getElementById('reply-to-post-id');
            return input?.value || null;
        };
        window.SnakkDraftManager?.restoreDraft(discussionId, textarea, getReplyToPostId());
        // Start auto-save
    ...
    const reactionEmojis = { ThumbsUp: '👍', Heart: '❤️', Eyes: '👀', Crazy: '🤯' };
    const reactionTypeValues = { ThumbsUp: 1, Heart: 2, Eyes: 3, Crazy: 4 };
    // Maps PascalCase type names to data-count-* attribute suffixes
    const reactionDataKeys = { ThumbsUp: 'thumbsup', Heart: 'heart', Eyes: 'eyes', Crazy: 'crazy' };
    ...
    function getReactionCountsFromAttrs(el) {
        const counts = {};
    ...
    function hideReactionPicker() {
        ...
        document.querySelectorAll('[data-actions-forced]').forEach(el => {
    ...
    function setupReactionPickerHover() {
        const picker = document.getElementById('reaction-picker');
        if (!picker || picker.dataset.hoverBound)
            return;
    ...
    function toggleReactionPicker(postId) {
        ...
        const hoverWrapper = postArticle?.querySelector('.hidden.group-hover\\:flex');
        if (hoverWrapper) {
        ...
        const smileyPlaceholder = reactionsBar.querySelector('[data-reaction-placeholder]');
    ...
    async function loadReactionsForPost(postId) {
        ...
        try {
            ...
            const keyMap = { ThumbsUp: 'thumbsUp', Heart: 'heart', Eyes: 'eyes', Crazy: 'crazy' };
            const serverCounts = {};
    ...
    async function toggleFollowDiscussion(discussionId) {
        ...
        const followCache = window.SnakkFollowCache;
        if (followCache) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const cache = window.SnakkFollowCache;
                if (cache) {
    ...
    async function loadMorePosts(discussionId, currentUserId, isAuthenticated, isLocked) {
        if (postsIsLoading || !postsHasMoreItems)
            return;
        postsIsLoading = true;
        ...
        try {
            const response = await fetch(`/bff/discussions/${discussionId}/posts?offset=${postsCurrentOffset}&pageSize=${postsPageSize}`, { credentials: 'include' });
            if (!response.ok)
    ...
    function initStickySidebar() {
        ...
        function handleScroll() {
            if (!sidebar || sidebarOriginalTop === null)
                return;
            const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    ...
    document.addEventListener('click', async (e) => {
        ...
        switch (actionName) {
            ...
            case 'retry-load-posts':
                retryLoadPosts(action.dataset.discussionId || '', action.dataset.currentUserId || '', action.dataset.isAuthenticated === 'true', action.dataset.isLocked === 'true', action.dataset.preferEndlessScroll === 'true');
                break;
            case 'load-more-posts':
                await loadMorePosts(action.dataset.discussionId || '', action.dataset.currentUserId || '', action.dataset.isAuthenticated === 'true', action.dataset.isLocked === 'true');
                break;
            // Report actions
            case 'open-report-modal':
                openReportModal(action.dataset.type || '', action.dataset.targetId || '', action.dataset.description || '', action.dataset.spaceId);
                break;
        }
    });
