...
(function () {
    ...
    function updateFollowUI() {
        ...
        if (!toggleBtn || !followText || !followIcon || !levelToggle || !discussionsBtn || !postsBtn)
            return;
        if (isFollowing) {
            // Following state
    ...
    async function loadMoreDiscussions() {
        ...
        if (loadMoreRequest || !hasMoreItems)
            return;
        const config = window.SnakkConfig;
        ...
        try {
            ...
            loadMoreRequest = fetch(`/bff/spaces/${spaceId}/discussions?offset=${currentOffset}&pageSize=${pageSize}`, { credentials: 'include' });
